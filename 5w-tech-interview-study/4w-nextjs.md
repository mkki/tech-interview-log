...
* 의도: 서버와 브라우저 사이의 네트워크 경계(Network Boundary)를 이해하고, 직렬화(Serialization) 불가능한 데이터(함수, Map, Set 등) 처리 경험 확인
...
> **참고**: `Date` 객체는 RSC의 Flight 프로토콜에서 네이티브로 직렬화를 지원하므로 서버→클라이언트 props 전달이 가능합니다. 단, 일반 JSON API 경계에서는 문자열 변환이 필요합니다.
...
```typescript
// Next.js 15부터 fetch의 기본 캐싱이 no-store(캐싱 안 함)로 변경되었습니다.
fetch(url, { cache: 'force-cache' });         // SSG처럼 동작 (명시적 캐싱)
fetch(url, { cache: 'no-store' });            // SSR처럼 동작 (Next.js 15 기본값)
...
> **참고**: Next.js 15부터 `unstable_cache`를 대체하는 `"use cache"` directive가 실험적으로 도입되었습니다. 향후 안정화되면 `"use cache"`를 사용하는 것이 권장됩니다.
...
- **위치**: 서버, 영구적 저장소
- **동작**: `fetch` 응답을 **요청 간에도 유지**되는 캐시에 저장합니다. `cache: 'force-cache'`로 영구 캐싱(Next.js 14까지 기본값, 15부터는 `no-store`가 기본), `next: { revalidate: N }`으로 시간 기반 재검증이 가능합니다.
