...
type Union = A | B;        // name 또는 age 중 공통된 것만 접근 가능
...
- **JS `#private`**: ECMAScript 표준으로, **런타임에서 실제로 은닉**됩니다. 트랜스파일 후에도 외부에서 접근하면 SyntaxError가 발생합니다.
...
1. **타입 매개변수가 한 곳에서만 사용되는 경우**: `<T>(x: T): void`는 `(x: unknown): void`와 동일하므로 제네릭이 불필요합니다.
